    ...
    function convertRun(run, messages, options) {
        ...
        if (run.color) {
            if (paths.length > 0) {
                ...
                lastPath.attributes["style"] = lastPath.attributes["style"] === undefined ? `color: #${run.color}` : `${lastPath.attributes["style"]}; color: #${run.color}`;
            } else {
                paths.push(htmlPaths.element('p', {style: `color: #${run.color}`}, {fresh: false}));
            }
        }
        if (run.backgroundColor) {
            if (paths.length > 0) {
                const lastPath = paths[paths.length - 1];
                lastPath.attributes["style"] = lastPath.attributes["style"] === undefined ? `background-color: #${run.backgroundColor}` : `${lastPath.attributes["style"]}; background-color: #${run.backgroundColor}`;
            } else {
                paths.push(htmlPaths.element('p', {style: `background-color: #${run.backgroundColor}`}, {fresh: false}));
            }
        }
